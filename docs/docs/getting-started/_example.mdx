...
- your project URL. Find `Meemaw.init` and update it with your project URL: 
    ```javascript title="client/src/app/tx.jsx"
    const meemaw = await Meemaw.init('https://getmeemaw.co/YOUR-PROJECT-URL');
    ```
    - Note: if you're self-hosting Meemaw locally, you don't need to change anything.
- the URL of your JSON-RPC API. Update the following line with your own URL:
    ...
    ```
- your Supabase URL and API Key. Modify `.env.local` in the `client/` folder, replace `YOUR_SUPABASE_URL` and `YOUR_SUPABASE_API_KEY` with yours.
